...
async function runPayroll() {
    await supabase.from('timesheets').update({ status: 'paid' }).eq('status', 'approved')
    fetchTimesheets()
  }
      ...
      <Card className="animate-fade-up delay-1" style={{ marginBottom: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <button style={{
            ...
            <Play size={14} /> onClick={runPayroll} disabled={ready===0} style={{width:'100%',padding:'13px',borderRadius:14,border:'none',background:'var(--p {ready} ready workers
