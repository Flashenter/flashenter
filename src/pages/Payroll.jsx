    ...
    <div>
      ...
      {loading ? (
        ...
        <>
              ...
              <div key={row.id} style={{ display: 'grid', gridTemplateColumns: '1fr 80px 60px 100px 100px 120px', gap: 8, padding: '10px 16px', alignItems: 'center', borderBottom: i < records.length - 1 ? '0.5px solid rgba(0,0,0,0.05)' : 'none' }}>
                ...
                <div style={{ display: 'flex', gap: 5 }}>
                  {row.status === 'pending' ? (
                    ...
                    <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 20, background: row.status === 'paid' ? 'var(--green-50)' : row.status === 'approved' ? '#EEEDFE' : 'var(--amber-50)', color: row.status === 'paid' ? 'var(--green-600)' : row.status === 'approved' ? '#534AB7' : 'var(--amber-600)' }}>{row.status}</span>
                  )}
                </div>
              </div>
            ))}
          ...
          {ready > 0 && (
            <button style={{ width: '100%', padding: '13px', borderRadius: 14, border: 'none', background: '#534AB7', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--font-sans)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8 }}>
