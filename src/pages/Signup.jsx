  ...
  if (success) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#F5F4F1', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ width: 440, background: '#fff', borderRadius: 20, padding: 40, boxShadow: '0 4px 40px rgba(0,0,0,0.08)', textAlign: 'center' }}>
        ...
        <a href="/" style={{ display: 'block', padding: '12px', borderRadius: 40, background: '#534AB7', color: '#fff', fontSize: 14, fontWeight: 600, textDecoration: 'none', textAlign: 'center' }}>
          Go to Flashenter
        </a>
      </div>
    </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#F5F4F1', fontFamily: 'DM Sans, sans-serif' }}>
      <div style={{ background: '#fff', borderBottom: '0.5px solid rgba(0,0,0,0.08)', padding: '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <a href="/login" style={{ fontSize: 13, color: '#534AB7', textDecoration: 'none', fontWeight: 500 }}>Already have an account? Sign in</a>
      </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 32 }}>
              {plans.map(plan => (
                <div key={plan.id} onClick={() => setSelectedPlan(plan.id)}
                  ...
                  {plan.popular && (
                    <div style={{ position: 'absolute', top: -12, left: '50%', transform: 'translateX(-50%)', background: '#534AB7', color: '#fff', fontSize: 10, fontWeight: 600, padding: '3px 12px', borderRadius: 20, whiteSpace: 'nowrap' }}>Most popular</div>
                  )}
                  {plan.limited && (
                    <div style={{ background: '#FAEEDA', color: '#854F0B', fontSize: 9, fontWeight: 600, padding: '2px 8px', borderRadius: 20, display: 'inline-block', marginBottom: 6 }}>Limited time offer</div>
                  )}
                  ...
                  {plan.monthlyPrice === 0 ? (
                    ...
                    <div style={{ marginBottom: 4 }}>
                      {plan.limited && <div style={{ fontSize: 12, color: '#B4B2A9', textDecoration: 'line-through' }}>${billing === 'monthly' ? plan.regularMonthly : (plan.regularMonthly * 10).toFixed(2)}</div>}
                      <div style={{ fontSize: 26, fontWeight: 700, color: '#534AB7' }}>
                        ${billing === 'monthly' ? plan.monthlyPrice : plan.annualPrice}
                        <span style={{ fontSize: 12, color: '#888780', fontWeight: 400 }}>{billing === 'monthly' ? '/mo' : '/yr'}</span>
                      </div>
                      {billing === 'annual' && <div style={{ fontSize: 11, color: '#3B6D11' }}>${plan.monthlyPrice}/mo · 2 months free!</div>}
                    </div>
                  )}
                  ...
                  {selectedPlan === plan.id && (
                    <div style={{ marginTop: 14, padding: '6px', borderRadius: 8, background: '#EEEDFE', color: '#534AB7', fontSize: 11, fontWeight: 600, textAlign: 'center' }}>Selected ✓</div>
                  )}
                </div>
              ))}
            </div>
            <div style={{ textAlign: 'center' }}>
              <button onClick={() => setStep(2)} style={{ padding: '13px 40px', borderRadius: 40, border: 'none', background: '#534AB7', color: '#fff', fontSize: 15, fontWeight: 600, cursor: 'pointer', fontFamily: 'DM Sans, sans-serif' }}>
