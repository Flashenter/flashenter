...
import { Mail, Phone, Edit, Users, Flame, Building, Clock, Plus, Search, Trash2 } from 'lucide-react'
...
export default function Contacts() {
  ...
  const [newContact, setNewContact] = useState({ name: '', title: '', company: '', country: '', email: '', phone: '', budget: '', status: 'new', stage: 'New inquiry' })
  ...
  return (
    <div>
      <PageHeader title="Contacts & leads" subtitle={`${contacts.length} contacts · 5 markets`}>
        <Button icon={Plus} variant="primary" onClick={() => setShowAdd(true)}>Add contact</Button>
      </PageHeader>

      {showAdd && (
        <Card style={{ marginBottom: 14, border: '1.5px solid var(--purple-200)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 10 }}>
            {[
              ...
              { key: 'country', label: 'Country (e.g. 🇩🇴 Dominican Republic)' },
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <Button variant="primary" onClick={addContact}>Save contact</Button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
        <MetricCard icon={Users} label="All contacts" value={contacts.length} sub="Total in database" />
        ...
        <MetricCard icon={Clock} label="New this week" value={contacts.filter(c=>c.status==='new').length} sub="New inquiries" />
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, alignItems: 'center' }}>
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, background: '#fff', border: '0.5px solid rgba(0,0,0,0.08)', borderRadius: 40, padding: '8px 14px' }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search contacts..." style={{ border: 'none', outline: 'none', fontSize: 13, background: 'transparent', flex: 1, fontFamily: 'var(--font-sans)' }} />
        </div>
        {filters.map(f => (
      ...
      {loading ? (
        <div style={{ textAlign: 'center', padding: 40, color: '#888780' }}>Loading contacts...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 60, background: '#fff', borderRadius: 16, border: '0.5px solid rgba(0,0,0,0.08)' }}>
          ...
          <Button variant="primary" icon={Plus} onClick={() => setShowAdd(true)}>Add your first contact</Button>
        </div>
      ) : (
              ...
              <Card key={c.id} style={{ cursor: 'pointer' }} onClick={() => navigate(`/contacts/${c.id}`)}>
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    ...
                    <div onClick={e => deleteContact(c.id, e)} style={{ width: 24, height: 24, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: 'var(--red-50)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#5F5E5A' }}><Mail size={11} color="#B4B2A9" />{c.email}</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 11, color: '#5F5E5A' }}><Users size={11} color="#B4B2A9" />{c.budget}</div>
                </div>
